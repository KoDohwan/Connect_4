...
int heuristic_function(int arr[][7], int turn) {                                    //노드에 대한 휴리스틱 값을 찾아주는 함수이다. --아이디어 회의: 고도환 조동혁, 코드 작성: 고도환 주석: 조동혁
	...
	for (i = 0; i <= 4; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 자신의 돌이 2개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 5; j++) {
	...
	for (i = 0; i <= 3; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 자신의 돌이 3개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 4; j++) {
	...
	for (i = 0; i <= 2; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 자신의 돌이 4개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 3; j++) {
	...
	for (i = 5; i >= 1; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우->좌) 방향으로 자신의 돌이 2개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 5; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == turn) {
				value += (winning_line[i][j] + winning_line[i - 1][j + 1]) * m2;
				i -= 2;
	...
	for (i = 5; i >= 2; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우>좌) 방향으로 자신의 돌이 3개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 4; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == arr[i - 2][j + 2] && arr[i][j] == turn) {
				value += (winning_line[i][j] + winning_line[i - 1][j + 1] + winning_line[i - 2][j + 2]) * m3;
			}
	...
	for (i = 5; i >= 3; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우->좌) 방향으로 자신의 돌이 4개 이어져 있는 경우에 value값에 가중치 부여.
		for (j = 0; j <= 3; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == arr[i - 2][j + 2] && arr[i][j] == arr[i - 3][j + 3] && arr[i][j] == turn) {
				value += (winning_line[i][j] + winning_line[i - 1][j + 1] + winning_line[i - 2][j + 2] + winning_line[i - 3][j + 3]) * m4;
			}
	...
	for (i = 0; i <= 4; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 상대의 돌이 2개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 5; j++) {
			if (arr[i][j] == arr[i + 1][j + 1] && arr[i][j] == turn) {
	...
	for (i = 0; i <= 3; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 상대의 돌이 3개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 4; j++) {
			if (arr[i][j] == arr[i + 1][j + 1] && arr[i][j] == arr[i + 2][j + 2] && arr[i][j] == turn) {
	...
	for (i = 0; i <= 2; i++) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(좌->우) 방향으로 상대의 돌이 4개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 3; j++) {
			if (arr[i][j] == arr[i + 1][j + 1] && arr[i][j] == arr[i + 2][j + 2] && arr[i][j] == arr[i + 3][j + 3] && arr[i][j] == turn) {
	...
	for (i = 5; i >= 1; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우->좌) 방향으로 상대의 돌이 2개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 5; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == turn) {
				value -= (winning_line[i][j] + winning_line[i - 1][j + 1]) * y2;
				i -= 2;
	...
	for (i = 5; i >= 2; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우->좌) 방향으로 상대의 돌이 3개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 4; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == arr[i - 2][j + 2] && arr[i][j] == turn) {
				value -= (winning_line[i][j] + winning_line[i - 1][j + 1] + winning_line[i - 2][j + 2]) * y3;
			}
	...
	for (i = 5; i >= 3; i--) {                                                         //7수 앞을 내다본 보드 상태에서 대각선(우->좌) 방향으로 상대의 돌이 4개 이어져 있는 경우에 value값에서 가중치 빼준다.
		for (j = 0; j <= 3; j++) {
			if (arr[i][j] == arr[i - 1][j + 1] && arr[i][j] == arr[i - 2][j + 2] && arr[i][j] == arr[i - 3][j + 3] && arr[i][j] == turn) {
				value -= (winning_line[i][j] + winning_line[i - 1][j + 1] + winning_line[i - 2][j + 2] + winning_line[i - 3][j + 3]) * y4;
			}
